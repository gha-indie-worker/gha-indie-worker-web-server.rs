...
pub fn load_from(lookup: impl Fn(&str) -> Option<String>) -> CliEnvValues {
    CliEnvValues {
        api_base: lookup("GHA_INDIE_WORKER_API_BASE").filter(|value| !value.is_empty()),
...
pub fn load_env_map(
    ...
    let mut out = std::collections::BTreeMap::new();
    let api_base = pick(&["GHA_INDIE_WORKER_API_BASE"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = api_base {
        out.insert("GHA_INDIE_WORKER_API_BASE".to_string(), value);
    }
    let config = pick(&["GHA_INDIE_WORKER_CONFIG"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = config {
        out.insert("GHA_INDIE_WORKER_CONFIG".to_string(), value);
    }
    let gha_indie_worker_api_http_base = pick(&["GHA_INDIE_WORKER_API_HTTP_BASE"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = gha_indie_worker_api_http_base {
        out.insert("GHA_INDIE_WORKER_API_HTTP_BASE".to_string(), value);
    }
    ...
    if let Some(value) = gha_indie_worker_web_bind {
        out.insert("GHA_INDIE_WORKER_WEB_BIND".to_string(), value);
    }
