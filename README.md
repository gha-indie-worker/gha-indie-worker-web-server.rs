...
Start-up is resolved and checked before anything binds: `src/flags.rs` runs
flags-2-env against `.cli-flags.toml` (an unknown or malformed option stops the
process), and `server::startup_plan` is the pure effect boundary. It validates
immutable configuration and derives non-sensitive backend capabilities before
`run` binds a socket; database connection strings never enter the printable
plan. `src/web_api_plane.rs` pins the four avenues to the shared
`k8s-web-api-data-plane` definitions, and `src/transport/nats.rs` carries the
web → API request plane on `dd.remote.web_api.gha-indie-worker.request` alongside
the `giw.*` event subjects.
...
`assets/vendor/htmx.min.js` and refuses the bytes unless they match the committed
SHA-384 pin (idempotent; safe to run in a Docker layer or in CI). Bumping htmx
is a deliberate two-line diff to `HTMX_VERSION` and `HTMX_SHA384`.
`ores-web-loader` is served from `/assets/loader/`, and content-addressed
...
src/
  ...
  flags.rs  web_api_plane.rs  (startup resolution; four-avenue data-plane binding)
```
