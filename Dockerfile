...
#
# gha-indie-worker-web-server — production container image.
#
...
#   docker buildx build --platform linux/arm64 -t ghcr.io/gha-indie-worker/gha-indie-worker-web-server:dev --load .
#
...
#     -t ghcr.io/gha-indie-worker/gha-indie-worker-web-server:dev --load .
#
#   # OCI image index with provenance and SBOM attestations:
#   docker buildx build --platform linux/arm64 --push \
#     --provenance=true --sbom=true \
#     --output type=image,oci-mediatypes=true,name=ghcr.io/gha-indie-worker/gha-indie-worker-web-server:dev .
#
#   `just docker-build` / `just docker-run` / `just docker-push` wrap all three.
#
# DEPENDENCIES — 0 crate(s) come from git remotes (Cargo.toml:
# none) and 0 from zed-pkg (.zpkg.toml: none).
...
#     ghcr.io/gha-indie-worker/gha-indie-worker-web-server:dev
#
...
COPY --from=builder --chown=65532:65532 /usr/local/bin/gha-indie-worker-web-server /usr/local/bin/gha-indie-worker-web-server
ENV GHA_INDIE_WORKER_WEB_BIND=0.0.0.0:8081
EXPOSE 8081
USER 65532:65532
ENTRYPOINT ["/usr/local/bin/gha-indie-worker-web-server"]
...
COPY --from=builder /usr/local/bin/gha-indie-worker-web-server /usr/local/bin/gha-indie-worker-web-server
