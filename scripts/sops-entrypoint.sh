...
#
# Presence is tested with `printenv`, not with an `eval` of a parameter
# expansion: a variable already present in the container environment wins, so
# an orchestrator can still override a single value (including the OTEL_* set
# by this image or by the Deployment) without re-encrypting.
