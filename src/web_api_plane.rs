//! Four-avenue web ↔ API binding for `gha-indie-worker`.
//!
//! 1. Direct read-only DB via `*-lib-core` named queries (no migrations).
//! 2. Stateless HTTP from `app.gha-indie-worker.dev` to `api.gha-indie-worker.dev`.
//! 3. Stateful TLS 1.3/mTLS TCP to `api.gha-indie-worker.dev:7443`.
...
pub const GITHUB_ORG: &str = "gha-indie-worker";
pub const ORG_SLUG: &str = "gha-indie-worker";
pub const DNS_ZONE: &str = "gha-indie-worker.dev";
