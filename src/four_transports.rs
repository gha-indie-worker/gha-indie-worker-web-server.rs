#![forbid(unsafe_code)]
//! The four avenues this web server can use to reach `gha-indie-worker-api-server`.
//!
...
//! to gha-indie-worker — the environment prefix, the service slug that names its NATS
//! subjects, and the api route that accepts an envelope.
//!
//! | # | Mode | What happens |
//! |---|------|--------------|
//! | 1 | `direct_read` | this server queries Postgres itself through `gha-indie-worker-lib-core`, read-only |
//! | 2 | `http` | stateless request to the `gha-indie-worker-api-server` cluster |
//! | 3 | `tcp` | framed, held-open connection to the `gha-indie-worker-api-server` cluster |
...
//! `gha-indie-worker-lib-core`'s read context, and hand it to [`gateway_from_env`]. See
...
///
/// `GHA_INDIE_WORKER_API_URL`, `GHA_INDIE_WORKER_NATS_URL`, and the rest; see the
/// table in `ores-transport`'s README.
pub const ENV_PREFIX: &str = "GHA_INDIE_WORKER";
...
pub const SERVICE_SLUG: &str = "gha-indie-worker";
